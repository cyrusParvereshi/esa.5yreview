...
fiveyr <- left_join(list_cnt, fiveyr_table, by = c("Scientific_Name" = "Species"))
fiveyr$Date <- as.Date(fiveyr$Date)
fiveyr <- filter(fiveyr, fiveyr$Date < as.Date("2017-01-31"))

filter_state <- function(state) {
  if(state != "All") {
    tmp <- filter(fiveyr, State == state)
  } else {
    tmp <- fiveyr
  }
  tmp <- distinct(tmp, Scientific_Name, State, .keep_all = TRUE)
...
filter_need5yr <- function(dat) {
  res <- filter(dat, First_Listed < THRES)
  return(res)
}
```
...
taxon <- unique(list_cnt$Species_Group)
taxon <- c("All", taxon)
inputPanel(
...
cur_dat <- reactive(filter_state(input$sel_state) %>% 
...
tool to evaluate the timeliness of five-year reviews. 
...
website. We will update the data on a regular basis and change the preceding 
date.")
```
...
df <- reactive({
  ...
  n_ontime <- sum(need_5yr$Review_Date >= as.Date("2012-01-01"))
  n_pastdue <- sum(need_5yr$Review_Date < as.Date("2012-01-01"))
